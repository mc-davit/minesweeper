const board: HTMLDivElement = document.querySelector("div.game-board") as HTMLDivElement;

// Special type for difficulty settings that takes:
// X and Y dimensions of the board (for example 10x8)
// Bomb Count
// Size of each cell in pixels
// Text, Flag and Bomb size in pixels
type Difficulty = { x: number, y: number, bombs: number, size: number, textSize: number };

// Pre-defined difficulty settings following Google's Minesweeper
const difficultySettings: { [key: string]: Difficulty} = {
...
let flagCount: HTMLSpanElement = document.querySelector("span#flag-count") as HTMLSpanElement;
let timeDisplay: HTMLSpanElement = document.querySelector('span#time') as HTMLSpanElement;
let difficultySelect: HTMLSelectElement = document.querySelector("select") as HTMLSelectElement;
...
let previousInnerCellContent: string;
...
function difficultySelection(): void {
    ...
    difficultySelect = document.querySelector("select") as HTMLSelectElement;
    ...
    let first: string = "evenish";
    let second: string = "oddish";
    ...
    cells.forEach((cell: HTMLDivElement, i: number) => {
        ...
        let calledIndices: Set<number> = new Set();
...
function createBombs(cells: NodeListOf<HTMLDivElement>) {
    const bombsSet: Set<number> = new Set();
 ...
 */
function emptyCells(cells: NodeListOf<HTMLDivElement>, index: number, calledIndices: Set<number>): void {
    ...
    neighborIndices.forEach(neighborIndex => {
        ...
        const notBombed: boolean = !cells[neighborIndex].classList.contains("bomb")
        const notFlagged: boolean = !cells[neighborIndex].classList.contains("notFlagged")
        ...
        if (calledIndices.has(neighborIndex)) return;
